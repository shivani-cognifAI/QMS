...
async function advanceCapaWorkflow(db, workflowId) {
  ...
  if (allApproved) {
    await db.prepare(`UPDATE capa_workflows SET status='Approved', completed_at=datetime('now') WHERE id=?`).run(workflowId);
    await db.prepare(`UPDATE capas SET approval_status='Approved', status='Approved & Closed', closed_at=date('now'), updated_at=datetime('now') WHERE id=?`).run(wf.capa_id);
  ...
  if (nextPending) {
    await db.prepare(`UPDATE capa_approval_steps SET status='Awaiting' WHERE id=?`).run(nextPending.id);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await db.transaction(async () => {
      await db.prepare(`UPDATE capa_approval_steps SET status='Approved', comment=?, acted_at=datetime('now') WHERE id=?`).run(req.body.comment || null, step.id);
