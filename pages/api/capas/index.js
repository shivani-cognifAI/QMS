...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const db = getDb();
      const existing = db.prepare('SELECT id FROM capas WHERE id = ?').get(id);
      if (existing) { db.close(); return res.status(409).json({ error: 'CAPA ID already exists' }); }

      let owner = null;
      if (owner_id) { const user = db.prepare('SELECT name FROM users WHERE id = ?').get(owner_id); if (user) owner = user.name; }

      db.prepare(`INSERT INTO capas (id, type, title, detail, clause, source, owner, owner_id, due_date, root_cause, action, status, pct_complete, created_by, updated_by) VALUES (@id, @type, @title, @detail, @clause, @source, @owner, @owner_id, @due_date, @root_cause, @action, @status, @pct_complete, @created_by, @created_by)`)
      ...
      for (const suffix of candidateSuffixes) {
        ...
        if (match) {
          ...
          const seqRow = db.prepare('SELECT last_seq FROM capa_seq WHERE record_type=? AND suffix=?').get(type, suffix);
          if (!seqRow) db.prepare('INSERT INTO capa_seq (record_type, suffix, last_seq) VALUES (?,?,?)').run(type, suffix, usedSeq);
          else if (usedSeq > seqRow.last_seq) db.prepare('UPDATE capa_seq SET last_seq=? WHERE record_type=? AND suffix=?').run(usedSeq, type, suffix);
          break;
        }
      }
