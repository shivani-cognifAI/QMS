...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const db = getDb();
      const existing = db.prepare('SELECT * FROM capas WHERE id = ?').get(id);
      ...
      const capa = db.prepare('SELECT * FROM capas WHERE id = ?').get(newId || id);
      db.close();
      return res.json(capa);
    }
    ...
    if (req.method === 'DELETE') {
      const db = getDb();
      const capa = db.prepare('SELECT id, status, approval_status FROM capas WHERE id = ?').get(id);
      ...
      db.prepare('DELETE FROM capas WHERE id = ?').run(id);
      db.close();
