...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      const db = getDb();
      const file = db.prepare('SELECT id FROM document_files WHERE id=?').get(id);
    ...
    if (req.method === 'GET') {
      const db = getDb();
      const files = db.prepare(`SELECT ${LIST_COLS} FROM document_files WHERE doc_id=? ORDER BY is_primary DESC, uploaded_at DESC`).all(id);
      db.close();
    ...
    if (req.method === 'POST') {
      ...
      const record = db.prepare(`SELECT ${LIST_COLS} FROM document_files WHERE file_hash=? AND doc_id=? ORDER BY uploaded_at DESC LIMIT 1`).get(fileHash, id);
      db.close();
