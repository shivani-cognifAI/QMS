...
export default async function handler(req, res) {
  ...
  try {
    ...
    const record = db.prepare(`SELECT ${LIST_COLS} FROM document_files WHERE file_hash=? AND entity_type='capa' AND entity_id=? ORDER BY uploaded_at DESC LIMIT 1`).get(fileHash, capaId);
    db.close();
