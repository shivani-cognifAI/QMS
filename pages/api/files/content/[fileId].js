...
export default async function handler(req, res) {
  ...
  try {
    ...
    db.prepare(`UPDATE document_files SET originalname=?, content_html=?, size=? WHERE id=?`).run(title?.trim() || file.originalname, html, (html || '').length, fileId);
    const updated = db.prepare('SELECT * FROM document_files WHERE id=?').get(fileId);
    db.close();
    res.json(updated);
  } catch (err) {
