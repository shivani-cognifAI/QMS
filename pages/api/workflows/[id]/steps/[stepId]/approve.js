...
function advanceWorkflow(db, workflowId) {
  ...
  if (allApproved) {
    ...
    db.prepare(`UPDATE workflows SET status='Approved', completed_at=datetime('now') WHERE id=?`).run(workflowId);

    if (isRetirement) {
      db.prepare(`UPDATE documents SET status='Retired', updated_at=datetime('now') WHERE id=?`).run(wf.doc_id);
      db.prepare(`INSERT INTO archived_versions (doc_id, doc_title, version, version_date, status, type, standard, clause, owner, review_date, scope, evidence, archived_at, archived_by, workflow_id, change_note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),?,?,?)`).run(doc.id, doc.title, doc.version, doc.version_date, 'Retired', doc.type, doc.standard, doc.clause, doc.owner, doc.review_date, doc.scope, doc.evidence, allApprovers, workflowId, `Document retired via approval workflow (${steps.length} approver${steps.length > 1 ? 's' : ''}: ${allApprovers}).`);
      db.prepare(`INSERT INTO version_history (doc_id, version, author, approved_by, change_note) VALUES (?,?,?,?,?)`).run(wf.doc_id, doc.version, wf.submitted_by, allApprovers, `Retirement approved via workflow. Document retired at v${doc.version}, effective ${approvedDate} (UTC). This document is now locked.`);
    ...
    db.prepare(`UPDATE documents SET status='Approved', version_date=?, updated_at=datetime('now') WHERE id=?`).run(approvedDate, wf.doc_id);
    db.prepare(`INSERT INTO version_history (doc_id, version, author, approved_by, change_note) VALUES (?,?,?,?,?)`).run(wf.doc_id, doc.version, wf.submitted_by, allApprovers, `Approved via sequential workflow (${steps.length} approver${steps.length > 1 ? 's' : ''}). Document approved at v${doc.version}, effective ${approvedDate} (UTC).`);
    db.prepare(`UPDATE version_history SET approved_by = ? WHERE id = (SELECT id FROM version_history WHERE doc_id = ? AND version = ? AND change_note NOT LIKE 'Submitted for sequential approval%' AND change_note NOT LIKE 'Approved via sequential workflow%' AND change_note NOT LIKE 'Rejected at step%' ORDER BY changed_at DESC LIMIT 1)`).run(allApprovers, wf.doc_id, doc.version);
  ...
  if (nextPending) {
    db.prepare(`UPDATE approval_steps SET status='Awaiting' WHERE id=?`).run(nextPending.id);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    db.transaction(() => {
      db.prepare(`UPDATE approval_steps SET status='Approved', comment=?, acted_at=datetime('now') WHERE id=?`).run(req.body.comment || null, step.id);
