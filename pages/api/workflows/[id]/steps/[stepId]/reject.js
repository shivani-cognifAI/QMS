...
export default async function handler(req, res) {
  ...
  try {
    ...
    await db.transaction(async () => {
      await db.prepare(`UPDATE approval_steps SET status='Rejected', comment=?, acted_at=datetime('now') WHERE id=?`).run(req.body.comment, step.id);
      await db.prepare(`UPDATE approval_steps SET status='Skipped' WHERE workflow_id=? AND status IN ('Pending','Awaiting') AND id != ?`).run(wf.id, step.id);
