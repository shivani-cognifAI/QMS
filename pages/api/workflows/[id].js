...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const db = getDb();
      const wf = getWorkflowFull(db, id);
      db.close();
    ...
    if (req.method === 'DELETE') {
      ...
      db.transaction(() => {
        db.prepare(`UPDATE workflows SET status='Cancelled', completed_at=datetime('now') WHERE id=?`).run(wf.id);
        db.prepare(`UPDATE documents SET status='Draft', updated_at=datetime('now') WHERE id=?`).run(wf.doc_id);
        db.prepare(`INSERT INTO version_history (doc_id, version, author, change_note) VALUES (?,?,?,?)`).run(wf.doc_id, wf.doc_version, 'System', 'Approval workflow cancelled — document returned to Draft');
      })();
      db.close();
