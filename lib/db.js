// Use the memory-growth asm.js build — grows heap on demand instead of pre-allocating 256MB
const initSqlJs = require('sql.js/dist/sql-asm-memory-growth.js');
const path = require('path');
const fs   = require('fs');

// On Vercel the bundle filesystem is read-only; writes must go to /tmp
const BUNDLE_DB_PATH = path.join(process.cwd(), 'data/qms.db');
const DB_PATH = process.env.VERCEL ? '/tmp/qms.db' : BUNDLE_DB_PATH;
...
function getDb() {
  ...
  } else if (process.env.VERCEL && fs.existsSync(BUNDLE_DB_PATH)) {
    // On first cold start on Vercel: seed /tmp from the read-only bundled db
...
async function initDb() {
  ...
  try { db.exec(`ALTER TABLE document_files ADD COLUMN file_data TEXT`); } catch(_) {}
  ...
  // Ensure a default admin exists so someone can always log in
  const hasAdmin = db.prepare("SELECT id FROM users WHERE system_role='admin' AND password_hash IS NOT NULL LIMIT 1").get();
  if (!hasAdmin) {
    const bcrypt = require('bcryptjs');
    const hash   = bcrypt.hashSync('Admin@1234', 10);
    db.prepare(`INSERT OR IGNORE INTO users (name,email,role,system_role,password_hash) VALUES ('Administrator','admin@qms.local','QA Manager','admin',?)`)
      .run(hash);
  }
...
// Singleton promise — resets on failure so the next request retries init
