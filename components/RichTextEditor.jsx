...
// Module-level cache so we only load once across all instances
...
function registerFormats(Quill) {
  ...
  try {
    ...
    console.warn('Quill format registration error:', e);
  }
}
...
export default function RichTextEditor({ value, onChange, placeholder, minHeight = 140 }) {
  ...
  // Load ReactQuill only in the browser, cache across instances
  useEffect(() => {
    if (_ReactQuill) {
      setLoaded(true);
      return;
    ...
    try {
      const rqMod = require('react-quill');
      _ReactQuill = rqMod.default || rqMod;
      require('react-quill/dist/quill.snow.css');

      // Quill is re-exported by react-quill or available from the quill package
      const rqQuill = rqMod.Quill;
      if (rqQuill && typeof rqQuill.import === 'function') {
        _Quill = rqQuill;
      } else {
        try {
          const qMod = require('quill');
          _Quill = qMod.default || qMod;
        } catch (_) {}
      }

      if (_Quill) registerFormats(_Quill);
    } catch (e) {
  ...
  // Set up toolbar handlers after editor mounts
  useEffect(() => {
    if (!loaded || !_Quill) return;
    ...
    try {
      ...
      function handleTableFocusOut(e) {
        ...
        const blot = _Quill.find(tableNode);
        if (!blot) return;
        const Delta = _Quill.import('delta');
      ...
      console.warn('Quill toolbar setup error:', e);
    }
  }, [loaded]);

  if (!loaded || !_ReactQuill) {
    return (
      <div style={{ minHeight, border: '1px solid var(--border-md)', borderRadius: 'var(--radius-md)', padding: 10, color: 'var(--text-3)', fontSize: 13 }}>
        Loading editor…
      </div>
    );
  }

  const ReactQuill = _ReactQuill;
